...
st.caption("※ 위쪽 몇 줄만 미리 보여줘요.")
st.dataframe(df.head(), use_container_width=True)

# 3️⃣ 이 가계부 형식(기간 / 분류 / 금액 등)에 맞게 컬럼 매핑
...
with c3:
    st.metric("수입 - 지출", f"{balance:,.0f} 원")
...
# 8️⃣ 이번 기간을 보면서 직접 작성하는 피드백 영역
st.subheader("6️⃣ 이번 기간 소비 피드백 작성하기 📝")

st.markdown(
    """
아래는 **이번 기간 지출을 보면서 정리하는 메모 구역**이에요.  
...
# 9️⃣ 복사해서 일기 / 노션 / 메모에 옮기기 좋은 텍스트 미리 보기
st.markdown("---")
...
if st.button("📋 요약 텍스트 만들기"):
    summary_lines = [
        ...
        f"- 총 수입: {total_income:,.0f} 원",
        f"- 총 지출: {total_expense:,.0f} 원",
        f"- 수입 - 지출: {balance:,.0f} 원",
        "",
        "## 😊 잘한 소비",
    ...
    st.success("아래 내용을 통째로 복사해서 일기/노션/메모에 붙여넣으면 좋아요!")
